    ...
    return (
        ...
        >
            <div className="flex flex-col items-center gap-2 justify-center py-10">
                <LazyImg className="w-[100%] h-[200px] object-cover" src={imageUrl} alt={`${title} services`} title={`${title} - ${description}`} />
                <h2 className="font-extrabold text-center text-gray-900 text-xl mb-2">{title}</h2>
                <p className="text-center text-sm font-light text-gray-600">{description}</p>
            </div>
        </a>
    );
};
